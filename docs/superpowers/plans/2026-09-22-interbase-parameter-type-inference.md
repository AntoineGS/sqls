...
- If the driver is developed in another linked worktree, use `/tmp/opencode/sqls-input-types/go.work` with `use` entries for sqls and the driver; workspace main modules override sqls's `go.mod` replacement. Because the home path contains `@`, make symlinks `/tmp/opencode/sqls-input-types/driver` and `/tmp/opencode/sqls-input-types/sqls` to the two worktrees, then from that directory run `go work init ./driver ./sqls`. Do **not** add a workspace `replace interbase-go` for a workspace main module: Go rejects it. Set `GOWORK` to the work-file path for tagged tests/builds and check `go list -m -json interbase-go` points to the driver symlink. Do not commit local paths or change the existing `../interbase-go` symlink.
