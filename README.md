...
metadata. An InterBase connection holds a single attachment: `showDatabases`
lists that attachment string, and `switchDatabase` accepts only that same name.
Configure a separate connection entry to open another database. InterBase has
no schema namespace, so `showSchemas` reports one synthetic empty schema.
